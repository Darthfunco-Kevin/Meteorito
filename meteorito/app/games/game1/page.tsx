  ...
  useEffect(() => {
    ...
    //Diseño epico
      ...
      app.ticker.add(() => {
        ...
        if (shipRef.current.x > app.screen.width - 30) shipRef.current.x = app.screen.width - 30;
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-b from-black via-indigo-950 to-purple-950 p-4">
      <div className="text-center mb-5">
        <h1 className="text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-blue-500 to-purple-600 mb-3" style={{ textShadow: '0 0 30px rgba(59, 130, 246, 0.5)' }}>
        ...
        <div className="flex justify-center items-center gap-8 mb-2">
          ...
          <div className="text-3xl font-bold">
            <span className={health > 50 ? 'text-green-400' : health > 25 ? 'text-yellow-400' : 'text-red-500'}>
              ❤️ Salud: {health}%
            </span>
          </div>
        </div>
        <div className="w-80 mx-auto h-6 bg-gray-800 rounded-full overflow-hidden border-2 border-gray-600">
          <div 
            className={`h-full transition-all duration-300 ${health > 50 ? 'bg-gradient-to-r from-green-400 to-green-600' : health > 25 ? 'bg-gradient-to-r from-yellow-400 to-orange-500' : 'bg-gradient-to-r from-red-500 to-red-700'}`}
            style={{ width: `${health}%` }}
          />
        </div>
      ...
      <div className="relative">
        <canvas ref={canvasRef} className="rounded-xl border-4 border-cyan-400 shadow-2xl shadow-cyan-500/50" style={{ boxShadow: '0 0 40px rgba(34, 211, 238, 0.4)' }} />
        
        {showGameOver && (
          <div className="absolute inset-0 bg-black bg-opacity-95 flex flex-col items-center justify-center rounded-xl border-4" style={{ backdropFilter: 'blur(10px)', borderColor: gameWon ? '#fcd34d' : '#ef4444' }}>
            <h2 className={`text-6xl font-bold text-transparent bg-clip-text mb-5 animate-pulse ${gameWon ? 'bg-gradient-to-r from-yellow-300 via-orange-400 to-red-500' : 'bg-gradient-to-r from-red-500 via-red-600 to-red-700'}`}>
            ...
            <button
              ...
              className={`px-12 py-5 text-2xl font-bold text-white rounded-xl transform hover:scale-110 transition-all shadow-lg animate-pulse ${gameWon ? 'bg-gradient-to-r from-green-400 via-emerald-500 to-teal-600 hover:from-green-500 hover:via-emerald-600 hover:to-teal-700 shadow-green-500/50' : 'bg-gradient-to-r from-blue-500 via-blue-600 to-blue-700 hover:from-blue-600 hover:via-blue-700 hover:to-blue-800 shadow-blue-500/50'}`}
            >
      ...
      <div className="mt-6 p-4 bg-gradient-to-r from-indigo-900/50 to-purple-900/50 rounded-lg border border-cyan-500/30 backdrop-blur-sm">
        ...
        <p className="text-white text-sm">🔹 Tecla <kbd className="px-2 py-1 bg-cyan-500/20 rounded border border-cyan-400">A</kbd> - Mover izquierda</p>
        <p className="text-white text-sm">🔹 Tecla <kbd className="px-2 py-1 bg-cyan-500/20 rounded border border-cyan-400">D</kbd> - Mover derecha</p>
        <p className="text-white text-sm">🔹 Tecla <kbd className="px-2 py-1 bg-cyan-500/20 rounded border border-cyan-400">ESPACIO</kbd> - Disparar láser</p>
        <p className="text-red-400 text-sm mt-2">⚠️ ¡Evita los meteoritos! Cada colisión quita 15% de salud</p>
        <p className="text-red-400 text-sm">⚠️ ¡No toques el suelo! Pierdes 20% de salud</p>
      </div>
    </div>
  );
}